function [es, ps] = scatteringProcess(obj, dv, es, ps, sc, pc)
    ...
    switch es.scatype
        ...
        case 2 % intra_ab_LA
            ps.aborem = "ab";
            ps.polar = "LA";
            k = dv.valley.chooseStandardVectorForInelasticScattering(es, pc, 'intra', sc.omegaLA(dv.valley.qAB), 1);
            es = dv.valley.getGeneralElectricWaveVector(es, pc, k);
        case 3 % intra_ab_TA
            ps.aborem = "ab";
            ps.polar = "TA";
            k = dv.valley.chooseStandardVectorForInelasticScattering(es, pc, 'intra', sc.omegaTA(dv.valley.qAB), 1);
            es = dv.valley.getGeneralElectricWaveVector(es, pc, k);
        case 4 % intra_em_LA
            ps.aborem = "em";
            ps.polar = "LA";
            while sc.omegaLA(dv.valley.qEM)*pc.hbar >= es.energy; dv.valley.qEM = dv.valley.qEM * 0.8; end
            k = dv.valley.chooseStandardVectorForInelasticScattering(es, pc, 'intra', sc.omegaLA(dv.valley.qEM), -1);
            es = dv.valley.getGeneralElectricWaveVector(es, pc, k);
        case 5 % intra_em_TA
            ps.aborem = "em";
            ps.polar = "TA";
            while sc.omegaTA(dv.valley.qEM)*pc.hbar >= es.energy; dv.valley.qEM = dv.valley.qEM * 0.8; end
            k = dv.valley.chooseStandardVectorForInelasticScattering(es, pc, 'intra', sc.omegaTA(dv.valley.qEM), -1);
            es = dv.valley.getGeneralElectricWaveVector(es, pc, k);
