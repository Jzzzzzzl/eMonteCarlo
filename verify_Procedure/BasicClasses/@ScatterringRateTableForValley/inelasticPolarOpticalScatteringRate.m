function inelasticPolarOpticalScatteringRate(obj, dv, es, pc, cc)
    %>生成极化光学散射率句柄函数
    %>     参数说明：
    %>     w：某光学支的声子平均频率
    %>
    ...
    obj.inelasticPolarOpticalAB = @(w) e^2*sqrt(dv.bs.md) * w/(sqrt(2)*pc.hbar*pc.epsilon0) ...
    ...
    obj.inelasticPolarOpticalEM = @(w) e^2*sqrt(dv.bs.md) * w/(sqrt(2)*pc.hbar*pc.epsilon0) ...
                                                  * (1/pc.epsilonH - 1/pc.epsilonL) ...
